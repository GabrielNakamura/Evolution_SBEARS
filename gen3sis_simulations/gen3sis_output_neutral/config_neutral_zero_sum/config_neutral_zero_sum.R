...
# Local carrying capacity: maximum number of individuals in the community.
K <- 1000
...
apply_ecology <- function(abundance, traits, environment, config) {
  ...
  # Compute the recruitment probability (relative abundances)
  recruitment_prob <- abundance/sum(abundance)
  
  # Sample the species in community K times.
  # K is the carrying capacity of the community, the maximum/total number of individuals.
  new_individuals <- sample(names(abundance), config$user$K, prob=recruitment_prob, replace=T)
  
  # Update the species abundances using a Zero-Sum game.
  abundance <- 0*abundance#rep(0, length(abundance))
  # Count the number of individuals of each species.
  new_individuals_by_species <- table(new_individuals) 
  abundance[names(new_individuals_by_species)] <- new_individuals_by_species
