...
#' @param eco.mut the eco.mut function handle
#'
#' @export
mutation_mode_temp <- function(evolving_traits,
                               trait_rules,
                               mutation_factor,
  ...
  mutation_mode_temp_internal <- function(species, cluster_indices, landscape, config) {
    ...
    # mutation prob. dependent on cell temperature
    mut_prob <- eco.mut(landscape[names(species[["abundance"]]), "temp"])
    mutations <- rbinom(n_clu_spi_ti,1,mut_prob)
  ...
  return(invisible(mutation_mode_temp_internal))
}
