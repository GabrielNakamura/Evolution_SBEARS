...
#' @param trait_evolutionary_power the amount of change per mutation
#'
#' @export
mutation_mode_abundance <- function(evolving_traits,
                                    trait_rules,
                                    mutation_factor,
  ...
  mutation_mode_abundance_internal <- function(species, cluster_indices, landscape, config) {
    ...
    #### discuss with LOIC P.!!!!
    #mut_prob <- lapply(traits_clusters, function (x) (config$exp$eco$mutation_factor^sum(x$"abd")))
    mut_prob <- lapply(abundance_clusters, function (abundance) {mutation_factor^sum(abundance)} )

    # applying mutation probability to each cell, ones are muttations!
    mutations <- lapply(1:length(traits_clusters), function(x) rbinom(nrow(traits_clusters[[x]]), 1, mut_prob[[x]]))
    # flatten mutations
    ...
    if (any(mutations)) { # if mutations hapened, then...
      ...
      # mutation mechanism TWO where only one trait mutate
      # #NOT DELETE!# mut_seq <- matrix(0, ncol=length(tr_evo), nrow=sum(mutations))
      # #NOT DELETE!# for (mut_i in 1:sum(mutations)){
      # #NOT DELETE!#   mut_seq[mut_i,sample(ncol(mut_seq),1)] <- sample(c(-tr_evo_pw, tr_evo_pw),1, replace=T)
      # #NOT DELETE!# }
      # #NOT DELETE!# NEW_clu_tr_spi[mutations,tr_evo] <- NEW_clu_tr_spi[mutations,tr_evo]+mut_seq
      ...
      # same as:
      # #avoid neg. values
      # NEW_clu_tr_spi[mutations,][NEW_clu_tr_spi[mutations,] < 0] <- 0
      # #avoid values bigger than 1
      # NEW_clu_tr_spi[mutations,][NEW_clu_tr_spi[mutations,] > 1] <- 1
  ...
  return(invisible(mutation_mode_abundance_internal))
}
